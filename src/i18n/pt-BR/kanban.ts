
export const kanbanTranslations = {
  title: "Quadro Kanban",
  manageColumns: "Gerenciar Colunas",
  addColumn: "Adicionar Coluna",
  newColumn: "Nova Coluna",
  columnTitle: "Título da Coluna",
  addTask: "Adicionar Tarefa",
  add: "Adicionar",
  cancel: "Cancelar",
  editColumn: "Renomear Coluna",
  noTasks: "Sem tarefas nesta coluna",
  clearColumn: "Limpar Coluna",
  clearColumnConfirm: "Tem certeza que deseja limpar esta coluna?",
  clearColumnDescription: "As tarefas removidas ficarão na Lixeira por 30 dias, depois serão permanentemente removidas.",
  clearColumnAction: "Sim, limpar coluna"
};

