
import { sidebarTranslations } from './sidebar';
import { dashboardTranslations } from './dashboard';
import { tasksTranslations } from './tasks';
import { taskModalTranslations } from './taskModal';
import { categoryManagerTranslations } from './categoryManager';
import { settingsTranslations } from './settings';
import { kanbanTranslations } from './kanban';
import { focusTranslations } from './focus';
import { focusSettingsTranslations } from './focusSettings';
import { pomodoroTranslations } from './pomodoro';

export const ptBRTranslations = {
  sidebar: sidebarTranslations,
  dashboard: dashboardTranslations,
  tasks: tasksTranslations,
  taskModal: taskModalTranslations,
  categoryManager: categoryManagerTranslations,
  settings: settingsTranslations,
  kanban: kanbanTranslations,
  focus: focusTranslations,
  focusSettings: focusSettingsTranslations,
  pomodoro: pomodoroTranslations
};
